...
#define STM32_GPIOA_BASE 0x48000000U /* 0x480000000: GPIOA/
...
#define STM32_GPIO_IDR_OFFSET 0x0010   // endereço de leitura 
...
#define RCC_AHB1ENR_GPIOCEN (1 << 2) /* Bit 2: IO port C clock
                                        enable */
#define RCC_AHB1ENR_GPIOAEN (1 << 0) /* Bit 0: IO port A clock
                                        enable */
...
#define GPIO_IDR_MASK(n)            (1 << GPIO_OT_SHIFT(n))
...
int main(int argc, char *argv[])
{
    ...
    /* Ponteiros para registradores  PortC*/
    ...
    /* Ponteiros para registradores  PortA*/
    uint32_t *pGPIOA_MODER = (uint32_t *)STM32_GPIOA_MODER;
    uint32_t *pGPIOA_PUPDR = (uint32_t *)STM32_GPIOA_PUPDR;
    uint32_t *pGPIOA_IDR = (uint32_t *)STM32_GPIOA_IDR;  // PONTEIRO DE LEITURA
   
    /* Habilita clock GPIOC e  GPIOA  */
    reg = *pRCC_AHB1ENR; // recebe o conteudo para salva-lo
    reg |= (RCC_AHB1ENR_GPIOCEN|RCC_AHB1ENR_GPIOAEN); // coloca 1 nos endereços  // reg |= (RCC_AHB1ENR_GPIOAEN);
    *pRCC_AHB1ENR = reg;
    ...
    *pGPIOA_PUPDR = reg;
    ...
    {
        uint32_t led_time;
        uint32_t sw_state = *pGPIOA_IDR & GPIO_IDR_MASK(0); 
        if(sw_state)
            led_time = 100;
        else 
            led_time = 500;
